...
let initEbState = {
    lastReadType: GridActionReader.readTypes.BOOK_LIST,
    // lastData: (lastSite?)
    lastBookId: null,
    //lastTimes: {}, // Video ID -> Player Time
    //lastEbookListIndexes: {}, // Playlist ID -> last played video index >>
    dataApiCalls: {},
};
...
let reader = null;
let readerID = 'player';
let ebState = localStorageService.getEBState() || JSON.parse(JSON.stringify(initEbState));
...
ebookService.doAction = function (action) {
    if (action.performAfterNav) {
        ebookService.setActionAfterNavigate(action);
        return;
    }
    switch (action.action) {
        case GridActionEbook.actions.YT_PLAY:
            youtubeService.play(action);
            break;
        case GridActionEbook.actions.YT_PAUSE:
            youtubeService.pause();
            break;
        case GridActionEbook.actions.YT_TOGGLE:
            youtubeService.toggle(action);
            break;
        case GridActionEbook.actions.YT_RESTART:
            youtubeService.restart(action);
            break;
        case GridActionEbook.actions.YT_STOP:
            youtubeService.stop();
            break;
        case GridActionEbook.actions.YT_STEP_FORWARD:
            youtubeService.seekToRelative(action.stepSeconds);
            break;
        case GridActionEbook.actions.YT_STEP_BACKWARD:
            youtubeService.seekToRelative(-action.stepSeconds);
            break;
        case GridActionEbook.actions.YT_NEXT_VIDEO:
            youtubeService.nextVideo();
            break;
        case GridActionEbook.actions.YT_PREV_VIDEO:
            youtubeService.previousVideo();
            break;
        case GridActionEbook.actions.YT_ENTER_FULLSCREEN:
            youtubeService.enterFullscreen();
            break;
        case GridActionEbook.actions.YT_VOLUME_UP:
            youtubeService.volumeUp(action.stepVolume);
            break;
        case GridActionEbook.actions.YT_VOLUME_DOWN:
            youtubeService.volumeDown(action.stepVolume);
            break;
        case GridActionEbook.actions.YT_VOLUME_MUTE:
